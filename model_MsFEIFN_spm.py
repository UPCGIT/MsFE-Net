...
class HSI_EMB(nn.Module):
    def __init__(self, hsi_channel, num_class, upscale_factor):
        ...
        self.conv_3 = nn.Conv2d(128, 100 * upscale_factor ** 2, (3, 3), stride=(1, 1), padding=1)
        self.BN_1 = nn.BatchNorm2d(256)
        self.BN_2 = nn.BatchNorm2d(128)
        self.BN_3 = nn.BatchNorm2d(100 * upscale_factor ** 2)
...
class Fusion_Unit(nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        self.conv_1 = nn.Conv2d(dim_in, dim_out, (3, 3), stride=(1, 1), padding=1)
        self.BN_1 = nn.BatchNorm2d(dim_out)
        self.deconv = nn.ConvTranspose2d(dim_out, dim_in, stride=1, kernel_size=3, padding=1) # output = (input-1)*stride + output_padding - 2padding + kernel_size
        self.BN_2 = nn.BatchNorm2d(dim_in)
        self.conv_2 = nn.Conv2d(dim_in, dim_out, (3, 3), stride=(1, 1), padding=1)
...
class Cross_Fusion(nn.Module):
    def __init__(self, dim_head, heads, cls):
        ...
        self.convH = nn.Conv2d(100, 128, (3, 3), stride=(1, 1), padding=1)
        self.BN_H1 = nn.BatchNorm2d(128)
        self.convL = nn.Conv2d(100, 128, (3, 3), stride=(1, 1), padding=1)
        ...
        self.FL = nn.Flatten()
        self.fcr = nn.Conv2d(512, cls, (3, 3), stride=(1, 1), padding=1)
    ...
    def forward(self, F_H, F_L):
        ...
        # classification
        x_result = F.softmax((self.fcr(F_M)), dim=1)

        return x_result


class MsFE(nn.Module):
    def __init__(self, hsi_channel, lidar_channel, cls, up):
        super(MsFE, self).__init__()
        self.L_EMB = LiDAR_EMB(lidar_channel)
        self.H_EMB = HSI_EMB(hsi_channel, cls, upscale_factor=up)
    ...
    def forward(self, x_H, y, x_L):

        x_H = torch.cat((x_H, y), dim=1)

        x_H= self.H_EMB(x_H)
        x_a, x_b, x_L = self.L_EMB(x_L)

        x_H = self.pixel_shuffle(x_H)
