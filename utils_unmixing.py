...
def load_mat_hsi(dataset_name, dataset_dir):
    ...
    if dataset_name == 'sa':
        ...
        image = io.loadmat(os.path.join(dataset_dir, "PaviaU.mat"))
        image = image['data']
        gt = io.loadmat(os.path.join(dataset_dir, "PaviaU_gt.mat"))
        gt = gt['gt']
        num_class = int(gt.max())
        labels = list(range(1, num_class + 1))

    elif dataset_name == 'india':
        image = io.loadmat(os.path.join(dataset_dir, "Indian_pines.mat"))
        image = image['data']
        gt = io.loadmat(os.path.join(dataset_dir, "Indian_pines_gt.mat"))
        gt = gt['gt']
        num_class = int(gt.max())
        labels = list(range(1, num_class + 1))
...
def label2color(label, data_name):
    ...
    if data_name == 'uni':
        ...
        map = [[202, 100, 95], [208, 190, 176], [255, 100, 0], [106, 179, 162], [0, 0, 255], [173, 54, 136],
               [149, 169, 56], [60, 91, 112], [255, 255, 0], [255, 255, 125], [255, 0, 255], [100, 0, 255],
...
def sample_gt(gt, train_size, mode="random", less_choice_per=0.4, seed=None):
    """
...
def plot_result(Data, patchsize, x2, gt, model, color_type, path, part=5000):
    [m, n, l] = np.shape(Data)
